...
public class transient_journals
	 ...
	 * not null: true<br>
	 */
	@Column(name = "fixed_at", type = 93, typeName = "timestamptz", size = 35, hasDecimalDigits = true, decimalDigits = 6, remarks = "移動時刻", defaultValue = "null", ordinalPosition = 4, notNull = true)
	 ...
	 * not null: true<br>
	 */
	@Column(name = "seq", type = -5, typeName = "bigserial", size = 19, hasDecimalDigits = true, decimalDigits = 0, remarks = "DB内生成順\nfixed_atが同一の場合、優先順を決定", defaultValue = "nextval('bb.transient_journals_seq_seq'::regclass)", ordinalPosition = 5, notNull = true)
	 ...
	 * @return このクラスのインスタンス
	 */
	public static transient_journals of(String id) {
		if (id == null || id.equals(""))
			throw new IllegalArgumentException("id が空です");

		return new transient_journals(getUsing(new Throwable().getStackTrace()[1]), id);
	}
	 ...
	 */
	public transient_journals(Optimizer optimizer) {
		selectBehavior().setOptimizer(Objects.requireNonNull(optimizer));
	}

	private transient_journals(Class<?> using, String id) {
	...
	public static class Assist<T, M> implements TableFacadeAssist {
		...
		private Assist(
			...
			this.extension = builder$.buildColumn(
				this,
				sqlassist.bb.transient_journals.extension);
