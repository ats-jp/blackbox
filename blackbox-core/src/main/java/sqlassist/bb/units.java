...
public class units
	 ...
	 * @return このクラスのインスタンス
	 */
	public static units of(String id) {
		if (id == null || id.equals(""))
			throw new IllegalArgumentException("id が空です");

		return new units(getUsing(new Throwable().getStackTrace()[1]), id);
	}
	 ...
	 */
	public units(Optimizer optimizer) {
		selectBehavior().setOptimizer(Objects.requireNonNull(optimizer));
	}

	private units(Class<?> using, String id) {
