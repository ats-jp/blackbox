 ...
 * type: TABLE<br>
 * remarks: アイテム<br>
 * 在庫管理する対象となる「もの」、SKU、個品など<br>
 */
@Table(name = "items", schema = "bb_stock", type = "TABLE", remarks = "アイテム\n在庫管理する対象となる「もの」、SKU、個品など")
...
public class items
	...
	public static class Assist<T, M> implements TableFacadeAssist {
		...
		private Assist(
			...
			this.id = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.id);
			this.group_id = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.group_id);
			this.name = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.name);
			this.revision = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.revision);
			this.extension = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.extension);
			this.tags = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.tags);
			this.active = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.active);
			this.created_at = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.created_at);
			this.created_by = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.created_by);
			this.updated_at = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.updated_at);
			this.updated_by = builder$.buildColumn(
				this,
				sqlassist.bb_stock.items.updated_by);
	...
	public static class ExtAssist<T, M> extends Assist<T, M> {
		 ...
		 */
		public sqlassist.bb.groups.ExtAssist<T, Many<sqlassist.bb_stock.items.Row, M>> $groups() {
			return new sqlassist.bb.groups.ExtAssist<>(
				builder$,
				this,
				sqlassist.bb_stock.items.bb$groups$items_group_id_fkey);
		}
		 ...
		 */
		public sqlassist.bb.users.ExtAssist<T, Many<sqlassist.bb_stock.items.Row, M>> $users$items_created_by_fkey() {
			return new sqlassist.bb.users.ExtAssist<>(
				builder$,
				this,
				sqlassist.bb_stock.items.bb$users$items_created_by_fkey);
		}
		 ...
		 * @return users relationship
		 */
		public sqlassist.bb.users.ExtAssist<T, Many<sqlassist.bb_stock.items.Row, M>> $users$items_updated_by_fkey() {
			return new sqlassist.bb.users.ExtAssist<>(
				builder$,
				this,
